...
class ScanlinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = Colors.black.withValues(alpha: 0.5)..strokeWidth = 2.0;
...
// ==========================================
// SCI-FI: HOLOGRAPHIC GLITCH TEXT
// ==========================================
class GlitchText extends StatelessWidget {
  ...
  const GlitchText({super.key, required this.text, required this.isGlitching, required this.style});
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Transform.translate(offset: const Offset(3, 0), child: Text(text, style: style.copyWith(color: Colors.redAccent.withValues(alpha: 0.8)))),
        Transform.translate(offset: const Offset(-3, 0), child: Text(text, style: style.copyWith(color: Colors.cyanAccent.withValues(alpha: 0.8)))),
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Future<void> _frageFortschrittSpeichern(int frageId, bool warRichtig) async {
    ...
    await db.insert('user_fortschritt', {
      'frage_id': frageId, 'korrekt_beantwortet': warRichtig ? 1 : 0, 'anzahl_versuche': versuche + 1,
      'letzter_versuch': jetzt, 'naechste_faelligkeit': naechsteFaelligkeit, 'intervall_tage': neuesIntervall,
  ...
  void _mcAntwortAuswerten(int index, dynamic istKorrektRaw) {
    ...
    setState(() {
      ...
      if (warRichtig) { _richtigeAntworten++; _aktuelleXP += 10; } 
      else { _falscheAntworten++; }
  ...
  void _freitextSelbstbewertung(bool warRichtig) {
    setState(() {
      if (warRichtig) { _richtigeAntworten++; _aktuelleXP += 15; } 
      else { _falscheAntworten++; }
      _pruefeAvatarEinsatz(warRichtig);
    });
    final frageId = _quizDaten[_aktuelleFrageIndex]['frage']['id'] as int;
  ...
  Widget _buildLiveStatsHUD() {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF1D2229), borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.2), blurRadius: 4, offset: const Offset(0, 2))]),
      child: Row(
  ...
  Widget _buildStatItem(IconData icon, Color color, String text) {
    return Row(children: [Icon(icon, color: color, size: 24), const SizedBox(width: 8), Text(text, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))]);
  }
        ...
        children: [
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // DER HOLOGRAPHISCHE GLITCH WENN FALSCH
                  GlitchText(
                    text: frage['frage_text'].toString(), 
                    isGlitching: _showErrorFlash, 
                  ...
                  if (frage['typ'] == 'multiple_choice') ...[
                    ...antworten.asMap().entries.map((entry) {
                      ...
                      if (_frageBeantwortet) {
                        if (istKorrekt == 1) { buttonColor = Colors.green.withValues(alpha: 0.2); textColor = Colors.greenAccent; border = const BorderSide(color: Colors.greenAccent, width: 2); } 
                        else if (_gewaehlteAntwortIndex == index) { buttonColor = Colors.red.withValues(alpha: 0.2); textColor = Colors.redAccent; border = const BorderSide(color: Colors.redAccent, width: 2); }
                      }
                    ...
                    if (_frageBeantwortet) ...[
                      ...
                      if (frage['erklaerung'] != null && frage['erklaerung'].toString().isNotEmpty)
                        Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFF1D2229), border: Border.all(color: Theme.of(context).colorScheme.primary.withValues(alpha: 0.4), width: 1.5), borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Theme.of(context).colorScheme.primary.withValues(alpha: 0.1), blurRadius: 10)]),
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(frage['erklaerung'].toString(), style: const TextStyle(color: Colors.white, fontSize: 15, height: 1.5)),
                            ],
                          ),
                        ),
                      const SizedBox(height: 20),
                    ...
                    TextField(controller: _freitextController, maxLines: 5, enabled: !_frageBeantwortet, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(labelText: 'Deine Antwort', border: OutlineInputBorder())),
                    const SizedBox(height: 20),
                    if (!_frageBeantwortet)
                      ElevatedButton.icon(style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)), onPressed: _freitextMusterloesungAnzeigen, icon: const Icon(Icons.check), label: const Text('Musterlösung prüfen', style: TextStyle(fontSize: 16)))
                    else ...[
                      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF121419), border: Border.all(color: Colors.greenAccent), borderRadius: BorderRadius.circular(12)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('✅ Musterlösung:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.greenAccent)), const SizedBox(height: 8), Text(frage['erklaerung']?.toString() ?? 'Keine Musterlösung hinterlegt.', style: const TextStyle(color: Colors.white70))])),
                      const SizedBox(height: 30),
                      ...
                      const SizedBox(height: 16),
                      Row(children: [Expanded(child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red.withValues(alpha: 0.2), foregroundColor: Colors.redAccent, padding: const EdgeInsets.symmetric(vertical: 16)), onPressed: () => _freitextSelbstbewertung(false), child: const Text('❌ Nein'))), const SizedBox(width: 16), Expanded(child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.green.withValues(alpha: 0.2), foregroundColor: Colors.greenAccent, padding: const EdgeInsets.symmetric(vertical: 16)), onPressed: () => _freitextSelbstbewertung(true), child: const Text('✅ Ja (+15 XP)')))]),
                    ]
                  ],
                ],
              ),
          ...
          AnimatedPositioned(
            ...
            child: Material(
              ...
              child: Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(color: const Color(0xFF1D2229), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.redAccent, width: 2), boxShadow: [BoxShadow(color: Colors.redAccent.withValues(alpha: 0.5), blurRadius: 15, spreadRadius: 2)]),
                child: Row(
                  children: [
                    ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.asset(_vaderGifPath, width: 80, height: 80, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => const Icon(Icons.person, color: Colors.white, size: 80))),
                    const SizedBox(width: 16),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Lord Vader sagt:', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold, fontSize: 14)), const SizedBox(height: 4), Text(_vaderSpruch, style: const TextStyle(color: Colors.white, fontSize: 16, fontStyle: FontStyle.italic))])),
                  ],
                ),
              ),
            ),
